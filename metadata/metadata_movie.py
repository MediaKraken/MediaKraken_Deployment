...
def movie_search_tmdb(db_connection, file_name):
    ...
    # search tmdb
    """
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie search tmdb: %s", file_name)
    ...
    if TMDB_CONNECTION is not None:
        ...
        common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie response: %s %s",
                     match_response, match_result)
        if match_response == 'idonly':
            ...
            common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie db result: %s", metadata_uuid)
        elif match_response == 'info':
            # store new metadata record and set uuid
            common_global.es_inst.com_elastic_index('info', {'stuff':
                "meta movie movielookup info results: %s", match_result)
        elif match_response == 're':
            # multiple results
            common_global.es_inst.com_elastic_index('info', {'stuff':"movielookup multiple results: %s", match_result)
    common_global.es_inst.com_elastic_index('info', {'stuff':'meta movie uuid %s, result %s', metadata_uuid, match_result)
...
def movie_fetch_save_tmdb(db_connection, tmdb_id, metadata_uuid):
    ...
    # fetch from tmdb
    """
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie tmdb save fetch: %s", tmdb_id)
    # fetch and save json data via tmdb id
    result_json = TMDB_CONNECTION.com_tmdb_metadata_by_id(tmdb_id)
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie code: %s", result_json.status_code)
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie save fetch result: %s", result_json.json())
    if result_json.status_code == 200:
        ...
        common_global.es_inst.com_elastic_index('info', {'stuff':"series: %s", series_id_json)
        # set and insert the record
        ...
        metadata_uuid = None
    common_global.es_inst.com_elastic_index('info', {'stuff':'meta movie save fetch uuid %s', metadata_uuid)
...
def movie_fetch_tmdb_imdb(imdb_id):
    ...
    common_global.es_inst.com_elastic_index('info', {'stuff':"uhimdb: %s", result_json)
...
def movie_fetch_save_tmdb_review(db_connection, tmdb_id):
    ...
    if review_json is not None and review_json['total_results'] > 0:
        ...
        common_global.es_inst.com_elastic_index('info', {'stuff':"review: %s", review_json_id)
...
def movie_fetch_save_tmdb_collection(db_connection, tmdb_collection_id, download_data):
    ...
    common_global.es_inst.com_elastic_index('info', {'stuff':"colfsdfsd: %s %s", tmdb_collection_id, collection_guid)
    if collection_guid is None:
        # insert
        collection_meta = TMDB_CONNECTION.com_tmdb_meta_collection_by_id(
            tmdb_collection_id)
        common_global.es_inst.com_elastic_index('info', {'stuff':"col: %s", collection_meta)
        # poster path
...
def metadata_movie_lookup(db_connection, media_file_path, download_que_json, download_que_id,
    ...
    common_global.es_inst.com_elastic_index('info', {'stuff':'meta movie look filename: %s', file_name)
    ...
    if 'year' in file_name:
        if file_name['title'] == metadata_movie_lookup.metadata_last_title \
            ...
            common_global.es_inst.com_elastic_index('info', {'stuff':'meta movie return 1 %s',
        ...
        db_connection.db_download_delete(download_que_id)
        common_global.es_inst.com_elastic_index('info', {'stuff':'meta movie return 2 %s',
                     metadata_movie_lookup.metadata_last_id)
        # don't need to set last......since they are equal
        return metadata_movie_lookup.metadata_last_id
    ...
    imdb_id, tmdb_id, rt_id = metadata_nfo_xml.nfo_xml_id_lookup(
        nfo_data, xml_data)
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie look: %s %s %s %s %s %s", imdb_id, tmdb_id, rt_id,
    ...
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie metadata_uuid A: %s", metadata_uuid)
        ...
        if tmdb_id is not None or imdb_id is not None:
            if tmdb_id is not None:
                ...
                if dl_meta is None:
                    ...
                    db_connection.db_download_delete(download_que_id)
                    metadata_uuid = dl_meta
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie metadata_uuid B: %s", metadata_uuid)
    if metadata_uuid is None:
        ...
        if metadata_uuid is not None:
            ...
            db_connection.db_download_update_provider(
                'themoviedb', download_que_id)
    common_global.es_inst.com_elastic_index('info', {'stuff':"meta movie metadata_uuid c: %s", metadata_uuid)
