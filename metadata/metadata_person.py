...
def metadata_fetch_tmdb_person(thread_db, provider_name, download_data):
    ...
    if TMDB_CONNECTION is not None:
        common_global.es_inst.com_elastic_index('info', {'stuff':"meta person tmdb save fetch: %s", download_data)
        # fetch and save json data via tmdb id
        result_json = TMDB_CONNECTION.com_tmdb_metadata_bio_by_id(
            download_data['mdq_download_json']['ProviderMetaID'])
        common_global.es_inst.com_elastic_index('info', {'stuff':"meta person code: %s", result_json.status_code)
        common_global.es_inst.com_elastic_index('info', {'stuff':"meta person save fetch result: %s", result_json.json())
