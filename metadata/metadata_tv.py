...
def tv_fetch_save_tvdb(db_connection, tvdb_id):
    ...
    if xml_show_data is not None:
        ...
        metadata_uuid = db_connection.srv_db_metatvdb_insert(series_id_json,\
        ...
        if xml_actor_data is not None:
            db_connection.srv_db_meta_person_insert_cast_crew('thetvdb',\
...
def metadata_tv_lookup(db_connection, media_file_path, download_que_json, download_que_id):
    ...
    if metadata_uuid is None:
        ...
        if 'year' in file_name:
            metadata_uuid = db_connection.srv_db_metatv_guid_by_tvshow_name(file_name['title'],\
                file_name['year'])
        else:
            metadata_uuid = db_connection.srv_db_metatv_guid_by_tvshow_name(file_name['title'],\
                None)
