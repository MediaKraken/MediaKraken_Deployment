...
base_cron = [
    # game subprograms
    ('Game Audit', 'Scan for new game media', './subprogram_game_audit'),
    # media/metadata subprograms
    ('Create Chapter Image', 'Create chapter images for all media', './subprogram_create_chapter_images'),
    ('Station Logo Fetch', 'Grab new logos from thelogodb', './subprogram_logo_download'),
    ('Anime', 'Match anime via Scudlee data', './subprogram_match_anime_id_scudlee'),
    ('Match Media', 'Match all new known media', './subprogram_match_known_media'),
    ('Roku Thumb', 'Generate Roku thumbnail images', './subprogram_roku_thumbnail_generate'),
    ('Schedules Direct', 'Fetch TV schedules from Schedules Direct', './subprogram_schedules_direct_updates'),
    ('Subtitle', 'Download missing subtitles for media', './subprogram_subtitle_downloader'),
    ('thetvdb Images', 'Grab missing thetvdb images', './subprogram_thetvdb_images'),
    ('thetvdb Update', 'Grab updated thetvdb metadata', './subprogram_thetvdb_updates'),
    ('The Movie Database', 'Grab updated movie metadata', './subprogram_tmdb_updates'),
    ('tvmaze Images', 'Grab missing tvmaze images', './subprogram_tvmaze_images'),
    ('tvmaze Update', 'Grab updated tvmaze metadata', './subprogram_tvmze_updates'),
    ('Collections', 'Create and update collection(s)', './subprogram_update_create_collections'),
    ('URL Check', 'Check all known URL', './subprogram_url_checker'),
    # normal subprograms
    ('Chromecast', 'Scan for chromecast device(s)', './subprogram_chromecast_discover'),
    ('Media Scan', 'Scan for new media', './subprogram_file_scan'),
    ('iRadio Scan', 'Scan for iRadio stations', './subprogram_iradio_channels'),
    ('Backup', 'Backup Postgresql DB', './subprogram_postgresql_backup'),
    ('DB Vacuum', 'Postgresql Vacuum Analyze all tables', './subprogram_postgresql_vacuum'),
    ('Sync', 'Sync/transcode media', './subprogram_sync'),
    ('Tuner', 'Scan for tuner device(s)', './subprogram_tuner_discover'),
    ('ZFS Check', 'Check local ZFS for failed drives', './subprogram_zfs_check'),
    ]
...
sql3_cursor.execute('create table IF NOT EXISTS mm_metadata_game_systems_info (gs_id uuid'\
    ' CONSTRAINT gs_id_mpk primary key, gs_game_system_json jsonb)')
...
if db_table_index_check('mm_game_systems_idxgin_name') is None:
    sql3_cursor.execute('CREATE INDEX mm_game_systems_idxgin_name'\
        ' ON mm_metadata_game_systems_info USING gin ((gs_game_system_json->\'@name\'))')


# create table for gamesdb systems
sql3_cursor.execute('create table IF NOT EXISTS mm_metadata_gamedb_systems_info (gsdb_id uuid'\
    ' CONSTRAINT gsdb_id_pk primary key, gsdb_game_system_id integer,'\
    ' gsdb_game_system_name text, gsdb_game_system_alias text, gsdb_game_system_json jsonb)')
