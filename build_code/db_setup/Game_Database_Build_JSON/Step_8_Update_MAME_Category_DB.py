...
while 1:
    ...
    if line.find("[") == 0:
        ...
        curs.execute('select gc_id from mm_game_category where gc_category = %s', quick_sql_args)
        for sql_row in curs:
...
for sql_row in curs.fetchall():
    ...
    try:
        ...
        curs.execute('update mm_game_info set gi_gc_category = %s where gi_id = %s', sql_args)
    except:
        pass
...
for sql_row in curs.fetchall():
    sql_args = sql_row[1],
    curs.execute('select gi_gc_category from mm_game_info where gi_short_name = %s', sql_args)
