...
total_media_to_match = db_connection.db_known_media_all_unmatched_count()
# begin the media match on NULL matches
for row_data in db_connection.db_known_media_all_unmatched():
    files_to_id += 1
...
# send notications
if files_to_id > 0:
    db_connection.db_notification_insert(locale.format('%d', files_to_id, True)\
        + " file(s) scanned.", True)
if files_added > 0:
    db_connection.db_notification_insert(locale.format('%d', files_added, True)\
        + " new media file(s) matched.", True)
if build_collection:
    db_connection.db_trigger_insert(('python', './subprogram_update_create_collections.py'))
