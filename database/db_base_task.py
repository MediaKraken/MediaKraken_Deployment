...
def db_task_time_update(self, task_type):
    ...
    """
    self.db_cursor.execute('update mm_task set mm_task_last_run = %s where mm_task_name = %s',
...
def db_task_info(self, task_uuid):
    ...
    self.db_cursor.execute('select mm_task_guid, mm_task_name,'
        ' mm_task_description, mm_task_enabled, mm_task_schedule, mm_task_last_run,'
        ' mm_task_file_path, mm_task_json from mm_task where mm_task_guid = %s', (task_uuid,))
