...
def MK_Server_Database_MetadataTV_GUID_By_IMDB(self, imdb_uuid):
    self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_media_tvshow_id->\'IMDB\' ? %s', (imdb_uuid,))
    try:
...
def MK_Server_Database_MetadataTV_GUID_By_TVDB(self, thetvdb_uuid):
    self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_media_tvshow_id->\'theTVDB\' ? %s', (thetvdb_uuid,))
    try:
        return self.sql3_cursor.fetchone()['mm_metadata_tvshow_guid']
    except:
        return None
...
def MK_Server_Database_MetadataTV_GUID_By_TVMaze(self, tvmaze_uuid):
    self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_media_tvshow_id->\'TVMaze\' ? %s', (tvmaze_uuid,))
    try:
        return self.sql3_cursor.fetchone()['mm_metadata_tvshow_guid']
    except:
        return None
...
def MK_Server_Database_MetadataTV_GUID_By_TVRage(self, tvrage_uuid):
    self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_media_tvshow_id->\'TVRage\' ? %s', (tvrage_uuid,))
    try:
        return self.sql3_cursor.fetchone()['mm_metadata_tvshow_guid']
    except:
        return None
...
# tvshow count
def MK_Server_Database_Metadata_TVShow_List_Count(self):
    self.sql3_cursor.execute(u'select count(*) from mm_metadata_tvshow')
    return self.sql3_cursor.fetchone()[0]


# return list of tvshows
def MK_Server_Database_Metadata_TVShow_List(self, offset=None, records=None):
    # COALESCE - priority over one column
    if offset is None:
        self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid,mm_metadata_tvshow_name, COALESCE(mm_metadata_tvshow_json->\'Meta\'->\'TVMaze\'->\'premiered\', mm_metadata_tvshow_json->\'Meta\'->\'theTVDB\'->\'Meta\'->\'Series\'->\'FirstAired\'), COALESCE(mm_metadata_tvshow_localimage_json->\'Images\'->\'TVMaze\'->>\'Poster\', mm_metadata_tvshow_localimage_json->\'Images\'->\'theTVDB\'->>\'Poster\') from mm_metadata_tvshow order by LOWER(mm_metadata_tvshow_name)')
    else:
...
def MK_Server_Database_Metadata_TVShow_Update_Image(self, image_json, metadata_uuid):
    self.sql3_cursor.execute(u'update mm_metadata_tvshow set mm_metadata_tvshow_localimage_json = %s where mm_metadata_tvshow_guid = %s', (image_json, metadata_uuid))


# fetch tvmaze rows to update
def MK_Server_Database_Metadata_TVShow_Images_To_Update(self, image_type):
    if image_type == 'TVMaze':
        self.sql3_cursor.execute(u"select mm_metadata_tvshow_json->\'Meta\'->\'TVMaze\',mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_tvshow_localimage_json->'Images'->'TVMaze'->'Redo' = 'true'")
    elif image_type == 'theTVDB':
        self.sql3_cursor.execute(u"select mm_metadata_tvshow_json->\'Meta\'->\'theTVDB\',mm_metadata_tvshow_guid from mm_metadata_tvshow where mm_metadata_tvshow_localimage_json->'Images'->'theTVDB'->'Redo' = 'true'")
    return self.sql3_cursor.fetchall()


# return metadata for tvshow
def MK_Server_Database_Metadata_TVShow_Detail(self, guid):
    self.sql3_cursor.execute(u'select mm_metadata_tvshow_name, mm_metadata_tvshow_json, mm_metadata_tvshow_localimage_json, COALESCE(mm_metadata_tvshow_localimage_json->\'Images\'->\'TVMaze\'->>\'Poster\', mm_metadata_tvshow_localimage_json->\'Images\'->\'theTVDB\'->>\'Poster\') from mm_metadata_tvshow where mm_metadata_tvshow_guid = %s', (guid,))
    try:
