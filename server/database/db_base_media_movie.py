...
def srv_db_web_media_list_count(self, class_guid, list_type=None, list_genre='All',
    ...
    if list_genre == 'All':
        if list_type == "recent_addition":
            if not group_collection:
                if not include_remote:
                    self.db_cursor.execute('select count(*) from (select distinct mm_metadata_guid from mm_media, mm_metadata_movie where mm_media_class_guid = %s and mm_media_metadata_guid = mm_metadata_guid and mm_media_json->>\'DateAdded\' >= %s) as temp', (class_guid, (datetime.datetime.now() - datetime.timedelta(days=7)).strftime("%Y-%m-%d"),))
                else:
                    self.db_cursor.execute('select count(*) from ((select distinct mm_metadata_guid from mm_media, mm_metadata_movie where mm_media_class_guid = %s and mm_media_metadata_guid = mm_metadata_guid and mm_media_json->>\'DateAdded\' >= %s) union (select distinct mmr_metadata_guid from mm_media_remote, mm_metadata_movie where mmr_media_class_guid = %s and mmr_media_metadata_guid = mm_metadata_guid and mm_media_json->>\'DateAdded\' >= %s)) as temp', (class_guid, (datetime.datetime.now() - datetime.timedelta(days=7)).strftime("%Y-%m-%d"), class_guid, (datetime.datetime.now() - datetime.timedelta(days=7)).strftime("%Y-%m-%d")))
            else:
                self.db_cursor.execute('select 1')
        else:
            if not group_collection:
                if not include_remote:
                    self.db_cursor.execute('select count(*) from (select distinct mm_metadata_guid from mm_media, mm_metadata_movie where mm_media_class_guid = %s and mm_media_metadata_guid = mm_metadata_guid) as temp', (class_guid,))
                else:
        ...
        if list_type == "recent_addition":
            ...
            if not group_collection:
                if not include_remote:
                    self.db_cursor.execute('select count(*) from (select distinct mm_metadata_guid from mm_media, mm_metadata_movie where mm_media_class_guid = %s and mm_media_metadata_guid = mm_metadata_guid and mm_metadata_json->\'genres\'->0->\'name\' ? %s) as temp', (class_guid, list_genre))
                else:
