...
def MK_Server_Database_Metadata_Person_List_Count(self):
    self.sql3_cursor.execute(u'select count(*) from mm_metadata_person')
...
def MK_Server_Database_Metadata_Person_List(self, offset=None, records=None):
    if offset is None:
        self.sql3_cursor.execute(u'select mmp_id,mmp_person_name, mmp_person_image from mm_metadata_person order by mmp_person_name')
    else:
        self.sql3_cursor.execute(u'select mmp_id,mmp_person_name,mmp_person_image from mm_metadata_person where mmp_id in (select mmp_id from mm_metadata_person order by mmp_person_name offset %s limit %s) order by mmp_person_name', (offset, records))
    return self.sql3_cursor.fetchall()


# return person data
def MK_Server_Database_Metadata_Person_By_GUID(self, guid):
    self.sql3_cursor.execute(u'select mmp_id, mmp_person_media_id, mmp_person_meta_json, mmp_person_image, mmp_person_name from mm_metadata_person where mmp_id = %s', (guid,))
    try:
...
def MK_Server_Database_Metadata_Person_By_Name(self, person_name):
    self.sql3_cursor.execute(u'select mmp_id, mmp_person_media_id, mmp_person_meta_json, mmp_person_image, mmp_person_name from mm_metadata_person where mmp_person_name = %s', (person_name,))
    try:
        return self.sql3_cursor.fetchone()
    except:
        return None
...
def MK_Server_Database_Metadata_Person_ID_Count(self, host_type, guid):
    ...
    #self.sql3_cursor.execute(u'select count(*) from mm_metadata_person where mmp_person_media_id @> \'{"Host":%s}\' and mmp_person_meta_json @> \'{"id":%s}\'', sql_params)
    self.sql3_cursor.execute(u'select count(*) from mm_metadata_person where mmp_person_media_id @> \'{"Host":"' + host_type + '"}\' and mmp_person_media_id @> \'{"id":%s}\'', (guid,))
    return self.sql3_cursor.fetchone()[0]
# works in postgresql
...
def MK_Server_Database_Metdata_Person_Insert(self, person_name, media_id_json, person_json, image_json=None):
    self.sql3_cursor.execute(u'insert into mm_metadata_person (mmp_id, mmp_person_name, mmp_person_media_id, mmp_person_meta_json, mmp_person_image) values (%s,%s,%s,%s,%s)', (str(uuid.uuid4()), person_name, media_id_json, person_json, image_json))
...
def MK_Server_Database_Metadata_Person_As_Seen_In(self, person_guid):
    ...
    if row_data['mmp_person_media_id']['Host'] == 'TMDB':
        ...
        self.sql3_cursor.execute(u'select mm_metadata_guid,mm_media_name,mm_metadata_localimage_json->\'Images\'->\'TMDB\'->\'Poster\' from mm_metadata_movie where mm_metadata_json->\'Meta\'->\'TMDB\'->\'Cast\' @> \'[{"id":%s}]\'', sql_params)
    elif row_data['mmp_person_media_id']['Host'] == 'TVMaze':
        sql_params = row_data['mmp_person_media_id']['id'],
        self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid,mm_metadata_tvshow_name,mm_metadata_tvshow_localimage_json->\'Images\'->\'TVMaze\'->\'Poster\' from mm_metadata_tvshow WHERE mm_metadata_tvshow_json->\'Meta\'->\'TVMaze\'->\'_embedded\'->\'cast\' @> \'[{"person": {"id": %s}}]\'', sql_params)  # TODO won't this need to be like below?
    elif row_data['mmp_person_media_id']['Host'] == 'theTVDB':
        #sql_params = str(row_data[1]['id']),
        self.sql3_cursor.execute(u'select mm_metadata_tvshow_guid,mm_metadata_tvshow_name,mm_metadata_tvshow_localimage_json->\'Images\'->\'theTVDB\'->\'Poster\' from mm_metadata_tvshow where mm_metadata_tvshow_json->\'Meta\'->\'theTVDB\'->\'Cast\'->\'Actor\' @> \'[{"id": \"' + str(row_data['mmp_person_media_id']['id']) + '\"}]\'')  #, sql_params)  #TODO
    return self.sql3_cursor.fetchall()
