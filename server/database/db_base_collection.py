...
def srv_db_Collection_List(self, offset=None, records=None):
    ...
    """
    if offset is None:
        self.sql3_cursor.execute('select mm_metadata_collection_guid,mm_metadata_collection_name,mm_metadata_collection_imagelocal_json from mm_metadata_collection order by mm_metadata_collection_name')
    else:
...
def srv_db_Media_Collection_Scan(self):
    ...
    """
    self.sql3_cursor.execute('select mm_metadata_guid, mm_metadata_json from mm_metadata_movie where mm_metadata_json->\'Meta\'->\'TMDB\'->\'Meta\'->>\'belongs_to_collection\'::text <> \'{}\'::text order by mm_metadata_json->\'Meta\'->\'TMDB\'->\'Meta\'->>\'belongs_to_collection\'')
    return self.sql3_cursor.fetchall()
...
def srv_db_Collection_GUID_By_Name(self, collection_name):
    ...
    """
    self.sql3_cursor.execute('select mm_metadata_collection_guid from mm_metadata_collection where mm_metadata_collection_name->>\'name\' = %s', (collection_name,))
    try:
...
def srv_db_Collection_By_TMDB(self, tmdb_id):
    ...
    """
    self.sql3_cursor.execute('select mm_metadata_collection_guid from mm_metadata_collection where mm_metadata_collection_json @> \'{"id":%s}\'', (tmdb_id,))
    try:
        return self.sql3_cursor.fetchone()['mm_metadata_collection_guid']
    except:
        return None
...
def srv_db_Collection_Read_By_GUID(self, media_uuid):
    ...
    Collection details
    """
    self.sql3_cursor.execute('select mm_metadata_collection_json,mm_metadata_collection_imagelocal_json from mm_metadata_collection where mm_metadata_collection_guid = %s', (media_uuid,))
    try:
