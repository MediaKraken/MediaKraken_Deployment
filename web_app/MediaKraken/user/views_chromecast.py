...
#        pass
        ...
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Stop', 'User': current_user.get_id()}))
        fpika.return_channel(ch)
    elif action == 'play':
        ch = fpika.channel()
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Play', 'User': current_user.get_id(),
                         'Data': g.db_connection.db_read_media(guid)['mm_media_path']}))
        fpika.return_channel(ch)
    elif action == 'pause':
        ch = fpika.channel()
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Pause', 'User': current_user.get_id()}))
        fpika.return_channel(ch)
#    elif action == 'ff':
#        pass
    elif action == 'forward':
        ...
        ch = fpika.channel()
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Mute', 'User': current_user.get_id()}))
        fpika.return_channel(ch)
    elif action == 'vol_up':
        ch = fpika.channel()
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Volume Up', 'User': current_user.get_id()}))
        fpika.return_channel(ch)
    elif action == 'vol down':
        ch = fpika.channel()
        ch.basic_publish(exchange='mkque_ex', routing_key='mkque',
                         body=json.dumps({'Type': 'Volume Down', 'User': current_user.get_id()}))
        fpika.return_channel(ch)
