...
def db_device_list(self, device_type=None, offset=0, records=None, search_value=None):
    ...
    """
    if device_type is None:
        self.db_cursor.execute('select mm_device_id,'
                               ' mm_device_type,'
                               ' mm_device_json'
                               ' from mm_device'
...
def db_device_update(self, guid, device_type, device_json):
    ...
    """
    self.db_cursor.execute('update mm_device set mm_device_type = %s,'
                           ' mm_device_json = %s'
...
def db_device_read(self, guid):
    ...
    """
    self.db_cursor.execute('select mm_device_type,'
                           ' mm_device_json'
                           ' from mm_device'
                           ' where mm_device_id = %s', (guid,))
    try:
