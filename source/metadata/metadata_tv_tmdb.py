...
async def tv_fetch_save_tmdb(db_connection, tmdb_id, metadata_uuid):
    ...
    """
    common_logging_elasticsearch_httpx.com_es_httpx_post(message_type='info', message_text= {"meta tv themoviedb save fetch": tmdb_id})
    result_json = common_global.api_instance.com_tmdb_metadata_tv_by_id(tmdb_id)
    common_logging_elasticsearch_httpx.com_es_httpx_post(message_type='info', message_text= {'tv fetch save themoviedb show': result_json})
    ...
    if result_json is None or result_json.status_code == 504:
        ...
        common_logging_elasticsearch_httpx.com_es_httpx_post(message_type='info', message_text= {"series": series_id})
