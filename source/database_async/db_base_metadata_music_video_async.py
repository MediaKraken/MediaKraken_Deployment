def db_meta_music_video_count(self, db_connection, imvdb_id=None, search_value=None):
    """
    Return count of music video metadata
    """
    if imvdb_id is None:
        if search_value is not None:
            return db_connection.fetchval('select count(*) from mm_metadata_music_video'
...
def db_meta_music_video_detail_uuid(self, db_connection, item_guid):
    """
    Grab metadata for specified music video
    """
    return db_connection.fetchrow('select mm_media_music_video_band,'
...
def db_meta_music_video_list(self, db_connection, offset=0, records=None, search_value=None):
    """
    List music video metadata
    """
    # TODO order by release date
    if search_value is not None:
        return db_connection.fetch('select mm_metadata_music_video_guid,'
