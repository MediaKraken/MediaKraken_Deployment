def db_meta_game_system_by_guid(self, db_connection, guid):
    """
    # return game system data
    """
    return db_connection.fetchrow('select * from mm_metadata_game_systems_info'
                                  ' where gs_id = %s',
                                  (guid,))


def db_meta_game_system_list_count(self, db_connection, search_value=None):
    """
    Return game system count
    """
    if search_value is not None:
        return db_connection.fetchval('select count(*) from mm_metadata_game_systems_info'
...
def db_meta_game_system_list(self, db_connection, offset=0, records=None, search_value=None):
    """
    # return list of game systems
    """
    # TODO might need to sort by release year as well for machines with multiple releases
    if search_value is not None:
        return db_connection.fetch('select gs_id,gs_game_system_name,'
