def db_meta_sports_guid_by_thesportsdb(self, db_connection, thesports_uuid):
    """
    # metadata guid by thesportsdb id
    """
    return db_connection.fetchval('select mm_metadata_sports_guid'
                                  ' from mm_metadata_sports'
                                  ' where mm_metadata_media_sports_id->\'thesportsdb\' ? %s',
                                  (thesports_uuid,))


def db_meta_sports_list(self, db_connection, offset=0, records=None, search_value=None):
    """
    # return list of sporting events
    # TODO order by year
    """
    if search_value is not None:
        return db_connection.fetch('select mm_metadata_sports_guid,'
        ...
        return db_connection.fetch('select mm_metadata_sports_guid, mm_metadata_sports_name'
...
def db_meta_sports_list_count(self, db_connection, search_value=None):
    """
    Count sport events
    """
    if search_value is not None:
        return db_connection.fetchval('select count(*) from mm_metadata_sports'
