...
def db_meta_person_as_seen_in(self, db_connection, person_guid):
    """
...
def db_meta_person_by_guid(self, db_connection, guid):
    """
    # return person data
    """
    return db_connection.fetch('select mmp_id, mmp_person_media_id,'
...
def db_meta_person_list(self, db_connection, offset=0, records=None, search_value=None):
    """
    # return list of people
    """
    # TODO order by birth date
    if search_value is not None:
        return db_connection.fetch('select mmp_id,mmp_person_name,'
                                   ' mmp_person_image,'
                                   ' mmp_person_meta_json->\'profile_path\' as mmp_meta'
        ...
        return db_connection.fetch('select mmp_id,mmp_person_name,mmp_person_image,'
                                   ' mmp_person_meta_json->\'profile_path\' as mmp_meta'
                                   ' from mm_metadata_person order by LOWER(mmp_person_name)'
                                   ' offset %s limit %s', (offset, records))


def db_meta_person_list_count(self, db_connection, search_value=None):
    """
    # count person metadata
    """
    if search_value is not None:
        return db_connection.fetchval('select count(*) from mm_metadata_person'
