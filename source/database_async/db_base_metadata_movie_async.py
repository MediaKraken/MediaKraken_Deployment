...
async def db_meta_movie_by_media_uuid(self, media_guid):
    ...
    return await self.db_connection.fetchrow('SELECT row_to_json(json_data)'
                                             ' FROM (select mm_metadata_json,'
                                             ' mm_metadata_localimage_json'
                                             ' from mm_media, mm_metadata_movie'
                                             ' where mm_media_metadata_guid = mm_metadata_guid'
                                             ' and mm_media_guid = $1) as json_data', media_guid)
...
async def db_meta_movie_detail(self, media_guid):
    ...
    return await self.db_connection.fetchrow('SELECT row_to_json(json_data)'
                                             ...
                                             ' where mm_metadata_guid = $1) as json_data',
                                             media_guid)
...
async def db_meta_movie_list(self, offset=0, records=None, search_value=None):
    ...
    if search_value is not None:
        return await self.db_connection.fetch('SELECT row_to_json(json_data)'
                                              ...
                                              ' order by mm_media_name, mm_date) as json_data',
...
async def db_meta_movie_status_update(self, metadata_guid, user_id, status_text):
    ...
    json_data = await self.db_connection.fetchrow('SELECT row_to_json(json_data)'
                                                  ' FROM (SELECT mm_metadata_user_json'
                                                  ' from mm_metadata_movie'
