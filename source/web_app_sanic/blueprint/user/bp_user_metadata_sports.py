...
async def url_bp_user_metadata_sports_detail(request, guid):
    ...
    return {
        'guid': guid,
        'data': await database_base_async.db_meta_sports_guid_by_thesportsdb(db_connection, guid)
    }
...
async def url_bp_user_metadata_sports_list(request):
    ...
    media = []
    for row_data in await database_base_async.db_meta_sports_list(db_connection,
    ...
    pagination = Pagination(request,
                            total=await database_base_async.db_meta_sports_list_count(db_connection,
    ...
    return {
        'media_sports_list': await database_base_async.db_meta_sports_list(db_connection, offset, per_page,
                                                                 request['session'][
                                                                     'search_text']),
        'page': page,
        'per_page': per_page,
