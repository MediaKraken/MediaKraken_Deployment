...
async def url_bp_user_media_new(request):
    ...
    media_data = []
    for media_file in await database_base_async.db_read_media_new(db_connection, offset, per_page,
    ...
    pagination = Pagination(request,
                            total=await database_base_async.db_read_media_new_count(db_connection,
                                request['session']['search_text'],
                                days_old=7),
