...
async def url_bp_user_metadata_periodical(request):
    ...
    item_list = []
    for item_data in await database_base_async.db_meta_book_list(db_connection, offset, per_page,
    ...
    pagination = Pagination(request,
                            total=await database_base_async.db_meta_book_list_count(db_connection,
                                request['session']['search_text']),
...
async def url_bp_user_metadata_periodical_detail(request, guid):
    ...
    """
    json_metadata = await database_base_async.db_meta_book_by_uuid(db_connection, guid)
    try:
