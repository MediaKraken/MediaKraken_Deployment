...
async def url_bp_user_queue_page(request, user):
    ...
    pagination = Pagination(request,
                            total=await request.app.db_functions.db_web_tvmedia_list_count(db_connection,
                                None, None,
    ...
    return {
        'media': await request.app.db_functions.db_meta_queue_list(db_connection, user.id, offset,
                                                    per_page,
                                                    request['session']['search_text']),
        'page': page,
        'per_page': per_page,
